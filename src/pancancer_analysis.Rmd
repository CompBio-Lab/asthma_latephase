...
p1 <- compVar(demoPre, t(genEset), variables, ncomp = 20)$p + ggtitle("Genes")
ggsave(here::here("results", "pca_demo_cors.png"), p1)
```
...
p2 <- ggplot(fev1.dat, aes(x = Time, y = fev1, group=Subj, colour=Col)) + geom_line() +
...
ggsave(here::here("results", "fev1_plot.png"), p2)
...
res <- mixOmics::splsda(X, Y, keep=c(25, 10))
cv <- perf(res, validation = "Mfold", folds = 5, nrepeat = 20,
           auc = TRUE)
panCancer <- unique(c(selectVar(res, comp=1)$name, selectVar(res, comp=2)$name))
saveRDS(panCancer, here::here("results", "panCancer_biomarkers.rds"))

df0 <- data.frame(value = as.numeric(cv$auc$comp2),
...
```{r}
result.mrna <- splsda(X = X, Y, keepX = rep(15, 2), ncomp = 2)

p3 <- result.mrna$variates$X %>% 
  as.data.frame() %>% 
  mutate(group = Y) %>% 
...
ggsave(here::here("results", "pancancer_comp_plot.png"), p3)
```

## plot var

```{r}
p4 <- rbind(selectVar(result.mrna, comp=1)$value,
...
ggsave(here::here("results", "pancancer_var_imp.png"), p4)
```

```{r}
p5 <- cowplot::plot_grid(plot_grid(p2, p3, labels = c("A", "B"), ncol = 1), p4, labels=c("", "C"))
ggsave(here::here("results", "Figure 1.png"), p5, width = 8, height = 8)
```
